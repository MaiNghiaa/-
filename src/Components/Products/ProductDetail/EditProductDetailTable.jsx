...
const EditProductDetailTable = ({
  productId,
  initialDetailValue,
  onClose,
  onUpdate,
}) => {
  const [values, setValues] = useState(initialDetailValue);

  const handleChange = (e) => {
    const { name, value } = e.target;
    setValues((prevDetail) => ({
      ...prevDetail,
      [name]: value,
    }));
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios
      .put(`http://localhost:3000/product_detail/${productId}`, values)
      .then((response) => {
...
export default EditProductDetailTable;
